...
def main(
    ...
    def island_group(slug: str, *, whole: bool) -> dict | None:
        """One archipelago, drawn in a box of its own.

        Islands are drawn where they are, at their true spacing. Packing them
        closer together fits far more land into the frame and was tried; it is
        not worth showing an archipelago in an arrangement it does not have.

        `whole` decides what the colour means. On the country page the coloured
        units are districts, and Madeira and the Azores *are* districts --- so
        each is one shape carrying one count, the same as Faro or Braga beside
        it. Colouring their municipalities there would have put two different
        levels of the hierarchy in one key. On the archipelago's own page it is
        the municipalities that are the subject, as everywhere else.
        """
        ...
        subs = (
            [sub_entry(region, packer)]
            if whole
            else [sub_entry(m, packer) for m in by_parent_m.get(region["osm_id"], [])]
        )
        return {
            **packer.header(),
            "name": region["name"],
            # With the region itself as the only shape, the focus outline would
            # be that same path stroked twice.
            "focus": [] if whole else packer.pack(region["rings"]),
            "subs": subs,
            "ctx": [],
        }
    ...
    country = {
        **cpack.header(),
        ...
        "insets": [
            g
            for g in (
                island_group(AZORES_SLUG, whole=True),
                island_group(MADEIRA_SLUG, whole=True),
            )
            if g
        ],
    }
    ...
    for region in regions.values():
        ...
        if region["slug"] in (AZORES_SLUG, MADEIRA_SLUG):
            payload = {**island_group(region["slug"], whole=False), "ctx": []}
        else:
            packer = Packer(viewport(region["bbox"]))
            payload = {
                **packer.header(),
